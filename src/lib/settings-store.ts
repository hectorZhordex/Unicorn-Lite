...
const DEFAULT: SiteSettings = {
  ...
  footerTagline: "BlueOrbit Technologies — Powering Your Digital Universe.",
  footerCopyright: "BlueOrbit Technologies. All rights reserved.",
...
export const useSettingsStore = create<SettingsStore>()(
  persist(
    (set) => ({
      ...
      clearAllData: () => {
        // Clear only uploads/stats data — NOT site settings
        if (typeof window !== "undefined") {
          const keysToRemove = Object.keys(localStorage).filter(
            (k) => k === "artflow-verification" || k === "blueorbit-auth" || k === "blueorbit_passwords" || k.startsWith("artflow_session")
          );
