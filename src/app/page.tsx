...
function HomeContent() {
  ...
  const fetchArtworks = useCallback(async (cat: string, q: string, reset = false) => {
    ...
    try {
      ...
      if (reset) {
        ...
        setArtworks((prev) => [...prev, ...(json.artworks || [])]);
  ...
  useEffect(() => {
    setOffset(0);
    fetchArtworks(category, searchQuery, true);
  }, [category, searchQuery]); // eslint-disable-line
  ...
  // Guest 1-minute timer
  useEffect(() => {
    if (currentUser) { setGuestBlur(false); if (timerRef.current) clearTimeout(timerRef.current); return; }
    timerRef.current = setTimeout(() => { setGuestBlur(true); setAuthOpen(true); }, GUEST_TIMEOUT * 1000);
    return () => { if (timerRef.current) clearTimeout(timerRef.current); };
  }, [currentUser]);

  const handleLoadMore = () => {
    setLoadingMore(true);
    fetchArtworks(category, searchQuery, false);
  };
  ...
  // After upload — refresh artworks so new file appears immediately
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className={guestBlur && !currentUser ? "pointer-events-none select-none" : ""}
        ...
        {/* Upload Banner — visible to everyone */}
            ...
            <div className="flex items-center gap-3 text-center sm:text-left">
              ...
              <div>
                <p className="text-sm font-semibold text-white">
                  {currentUser ? `Welcome, ${currentUser.name}! Share your designs.` : "Have a design to share? Upload your files for free."}
                </p>
                <p className="text-xs text-text-muted mt-0.5">Files are listed anonymously — your name is never shown publicly.</p>
              </div>
            </div>
            <button
              onClick={() => currentUser ? setUploadOpen(true) : setAuthOpen(true)}
        ...
        <section id="artworks" className="max-w-7xl mx-auto px-4 sm:px-6 mb-16">
          <div className="flex items-center justify-between gap-3 mb-6">
            <div className="flex items-center gap-3">
              ...
              {!loading && <span className="text-sm text-text-muted">({total})</span>}
            </div>
            {/* Refresh button */}
